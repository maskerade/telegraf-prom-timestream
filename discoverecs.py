...
class Main:
    ...
    def write_promjobs(self, promtargets):
        file_name = self.directory + '/' + 'input.prometheus.conf'
        tmp_file_name = file_name + '.tmp'
        with open(tmp_file_name, 'w') as f:
            f.write('[[inputs.prometheus]]\n')
            f.write('metric_version = 1\n\n')
            f.write('## An array of urls to scrape metrics from.\n')
            f.write('urls = ')

            f.write(json.dumps(promtargets))
            f.write('\n')
    ...
    def discover_tasks(self):
        ...
        promtargets = []
        promtargets.append('10.0.181.133')
        for target in targets:
            promtargets.append(target.ip)
        ...
        self.write_promjobs(promtargets)

    def discover_tasks_prom(self):
        targets = self.get_targets()
        log('Targets: ' + str(len(targets)))
        promtargets = []
        for target in targets:
            promtargets.append("http://%s/metrics/" %target.ip)
            log(promtargets)        
